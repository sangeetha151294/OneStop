			...
			try {
				...
				JSONObject profileInfo = getUserDetails(accessToken, json.getString("token_type"));
					if(profileInfo != null && !profileInfo.has("error")) {
						...
						String username=profileInfo.optString("displayName");
						
						jsonObject.append("name", username);
						request.getSession().setAttribute("userinfo", jsonObject);
						userDB udb=new userDB();
						int cid=udb.getUser(jsonObject.optString("email")).getCid();
						request.getSession().setAttribute("email",jsonObject.optString("email"));
						request.getSession().setAttribute("cid",cid);
						request.getSession().setAttribute("name",username);
						request.getSession().setAttribute("role",udb.getUser(jsonObject.optString("email")).getRole());
					}
